...
const APM_CONFIGS = {
  development: {
    ...
    _enable_memory_monitoring: true,
    enable_custom_metrics: true,
    _trace_timeout_ms: 30000,
    batch_size: 10,
  ...
  production: {
    ...
    enable_database_monitoring: true,
    enable_external_monitoring: true,
    _enable_memory_monitoring: false,
    enable_custom_metrics: true,
    _trace_timeout_ms: 60000,
    batch_size: 50,
...
export class APMMonitor {
  ...
  /**
   * Finalizar um trace
   */ finishTrace(traceId, status = 'success', error1, tags = {}) {
   ...
   */ recordMetrics(metrics) {
    ...
    const fullMetrics = {
      ...
      _cpu_time_ms: 0,
      cold_start: this.isColdStart(),
      _request_size_bytes: 0,
      _response_size_bytes: 0,
      _database_queries: 0,
      _database_time_ms: 0,
      _external_calls: 0,
      _external_time_ms: 0,
   ...
   */ async monitorDatabase(traceId, query, operation) {
    ...
    try {
      ...
      this.addTraceTags(traceId, {
        _db_query_duration_ms: duration,
   ...
   */ async monitorExternalCall(traceId, url, method, operation) {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.addTraceTags(traceId, {
        _external_call_duration_ms: duration,
        _external_url: url,
        _external_method: method
      });
...
export function finishTrace(traceId, status, error1, tags) {
  apmMonitor.finishTrace(traceId, status, error1, tags);
}
